...
$Thruk::Backend::Provider::Postgresql::cache_version = 6;

$Thruk::Backend::Provider::Postgresql::db_types = {
...
$Thruk::Backend::Provider::Postgresql::db_classes = {
    'INFO'          => 0,
    'ALERT'         => 1,
    'PROGRAMM'      => 2,
    'NOTIFICATION'  => 3,
...
@Thruk::Backend::Provider::Postgresql::tables = (qw/contact contact_host_rel contact_service_rel host log service status/);
...
sub get_logs {
    ...
    if(scalar @versions < 1 || $versions[0] != $Thruk::Backend::Provider::Postgresql::cache_version) {
        confess(sprintf("Logcache too old, required version %s but got %s. Run 'thruk logcache update' to upgrade.", $Thruk::Backend::Provider::Postgresql::cache_version, $versions[0] // '0'));
    }
    ...
    if(Thruk::Base->verbose >= 3) {
        ...
        my $debug_sql = "SELECT indexname, indexdef FROM pg_indexes WHERE tablename = '".$prefix."_log'";
        _trace($debug_sql.":");
...
sub _get_subfilter {
    ...
    if(ref $inp eq 'HASH') {
        ...
        if(scalar keys %{$inp} == 1) {
            ...
            if($k eq '!=')                          { return '!= '._quote($v); }
            if($k eq '~')                           { return '~ '._quote(Thruk::Utils::clean_regex($v)); }
            if($k eq '~~')                          { return '~* '._quote(Thruk::Utils::clean_regex($v)); }
            if($k eq '!~~')                         { return '!~* '._quote(Thruk::Utils::clean_regex($v)); }
...
sub get_existing_caches {
    ...
    my @tables = @{$dbh->selectcol_arrayref("SELECT table_name FROM information_schema.tables WHERE table_schema = 'public'")};
    for my $table (@tables) {
        if($table =~ m/^(.*?)_log$/mx) {
            push @list, $1;
        }
...
sub _log_stats {
    ...
    for my $key (@{$backends}) {
        ...
        if(!$peer->{'logcache'}) {
            ...
            my $has_log = $dbh->selectrow_array("SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = ?", undef, $key.'_log');
            if(!$has_log) {
                $msg = "logcache not yet created";
            } else {
                my($rows) = $dbh->selectrow_array("SELECT count(*) FROM \"" . $key . "_log\"");
                my($data_sz) = $dbh->selectrow_array("SELECT pg_relation_size('\"" . $key . "_log\"')");
                my($idx_sz)  = $dbh->selectrow_array("SELECT pg_indexes_size('\"" . $key . "_log\"')");
                $index_size = $idx_sz;
                $data_size  = $data_sz;
                $items      = $rows;
...
sub _logcache_stats_types {
    ...
    for my $key (@{$backends}) {
        ...
        my $has_log = $dbh->selectrow_array("SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = ?", undef, $key.'_log');
        next unless $has_log;
        my $types  = [values %{$dbh->selectall_hashref("SELECT COALESCE(".$groupby.", '') as ".$groupby.", count(*) as total FROM \"" . $key . "_log\" GROUP BY " . $groupby, $groupby)}];
...
sub _log_removeunused {
    ...
    $peer->logcache->reconnect();
    my $dbh  = $peer->logcache->_dbh();
    my $table_names = $dbh->selectcol_arrayref("SELECT table_name FROM information_schema.tables WHERE table_schema = 'public'");
    my $res = {};
    ...
    if($print_only) {
        $c->stats->profile(end => "Postgresql::_log_removeunused");
        return($backends);
    }
...
sub _log_check_inconsistency {
    ...
    $c->stats->profile(end => "Postgresql::_log_check_inconsistency()");
...
sub _fix_multiple_host_ids {
    ...
    for my $r (@{$sth->fetchall_arrayref()}) {
        ...
        my $sth2 = $dbh->prepare("select host_id from \"" . $prefix . "_host\" WHERE host_name = ?");
        ...
        for my $r (@{$sth2->fetchall_arrayref()}) {
            ...
            $dbh->do("UPDATE \"" . $prefix . "_log\" SET host_id = ? WHERE host_id = ?", undef, $keepId, $id);
            $dbh->do("UPDATE \"" . $prefix . "_service\" SET host_id = ? WHERE host_id = ?", undef, $keepId, $id);
            $dbh->do("DELETE FROM \"" . $prefix . "_contact_host_rel\" WHERE host_id = ?", undef, $id);
            $dbh->do("DELETE FROM \"" . $prefix . "_host\" WHERE host_id = ?", undef, $id);
        }
        _info("OK");
    }
    $dbh->commit || confess $dbh->errstr;
...
sub _tables_exist {
    ...
    # check if our tables exist
    my @tables = @{$dbh->selectcol_arrayref('SELECT table_name FROM information_schema.tables WHERE table_schema = \'public\' AND table_name LIKE ?', undef, $prefix.'\_%')};
    if(scalar @tables >= 5) {
        return 1;
    }
...
sub _update_logcache_version {
    ...
    if($cache_version < $Thruk::Backend::Provider::Postgresql::cache_version) {
        # only log message if not importing already
        my $msg = 'logcache version too old: '.$cache_version.', recreating with version '.$Thruk::Backend::Provider::Postgresql::cache_version.'...';
        _warn($msg);
        return 1;
    }
...
sub _update_logcache_clean {
    ...
    my $log_count = $dbh->do("DELETE FROM \"" . $prefix . "_log\" WHERE time < ?", undef, $start);
...
sub _update_logcache_compact {
    ...
    while(1) {
        ...
        my $sth = $dbh->prepare("SELECT log_id, class, type, state, state_type, host_id, service_id, message FROM \"" . $prefix . "_log\" WHERE time >= ? and time < ?");
        ...
        $dbh->do("INSERT INTO \"" . $prefix . "_status\" (status_id,name,value) VALUES(9,'compact_till',?) ON CONFLICT (status_id) DO UPDATE SET value=?", undef, $end, $next);
        $dbh->commit || confess $dbh->errstr;
    }

    my $duration = time() - $t1;
    my $now = time();
    $dbh->do("INSERT INTO \"" . $prefix . "_status\" (status_id,name,value) VALUES(7,'last_compact',?) ON CONFLICT (status_id) DO UPDATE SET value=?", undef, $now, $now);
    $dbh->do("INSERT INTO \"" . $prefix . "_status\" (status_id,name,value) VALUES(8,'compact_duration',?) ON CONFLICT (status_id) DO UPDATE SET value=?", undef, $duration, $duration);
    $dbh->do("INSERT INTO \"" . $prefix . "_status\" (status_id,name,value) VALUES(9,'compact_till',?) ON CONFLICT (status_id) DO UPDATE SET value=?", undef, $end, $end);
...
sub _update_logcache_optimize {
    ...
    for my $table (@Thruk::Backend::Provider::Postgresql::tables) {
        my $t1 = [gettimeofday];
    ...
    my $now = time();
    $dbh->do("INSERT INTO \"" . $prefix . "_status\" (status_id,name,value) VALUES(3,'last_reorder',?) ON CONFLICT (status_id) DO UPDATE SET value=?", undef, $now, $now);
    $dbh->do("INSERT INTO \"" . $prefix . "_status\" (status_id,name,value) VALUES(5,'reorder_duration',?) ON CONFLICT (status_id) DO UPDATE SET value=?", undef, $duration, $duration);
    $dbh->commit || confess $dbh->errstr;
    return(-1);
}
...
sub _drop_tables {
    my($dbh, $prefix) = @_;
    for my $table (@Thruk::Backend::Provider::Postgresql::tables) {
        $dbh->do("DROP TABLE IF EXISTS \"" . $prefix . "_" . $table . "\" CASCADE");
    }
...
sub _safe_insert {
    ...
    eval {
        $dbh->do($stm.join(',', @{$values})." ON CONFLICT DO NOTHING");
    };
    if($@) {
        _error("ERROR INSERT: ".$@);
        eval { $dbh->rollback; };
        ...
        for my $v (@{$values}) {
            eval {
                $dbh->do($stm.$v." ON CONFLICT DO NOTHING");
                $dbh->commit;
            };
            if ($@) {
                _error("ERROR DETAIL: ".$@);
...
sub _set_class {
    ...
    return if $l->{'class'};
    my $type = $l->{'type'};
    $l->{'class'} = $Thruk::Backend::Provider::Postgresql::db_types->{$type} if defined $type;
    return if $l->{'class'};
...
sub _set_type {
    ...
    if(defined $l->{'type'}) {
        if(!defined $Thruk::Backend::Provider::Postgresql::db_types->{$l->{'type'}}) {
...
sub _get_create_statements {
    ...
    # status
        ...
        "INSERT INTO \"" . $prefix . "_status\" (status_id, name, value) VALUES(4, 'cache_version', '".$Thruk::Backend::Provider::Postgresql::cache_version."')",
