...
sub handle_oauth_login {
    ...
    if($code && $state) {
        ...
        if ($auth->{'api_url'}) {
            ...
            ($login, $roles) = _extract_login($auth, $userinfo);
            if(!defined $login) {
            ...
            ($login, $roles) = _extract_login($auth, $id_token);
            if(!defined $login) {
...
sub _extract_login {
    ...
    if($auth->{'login_field'}) {
        return($userinfo->{$auth->{'login_field'}}, $roles);
    }

    return($userinfo->{'login'}, $roles) if $userinfo->{'login'};
    return($userinfo->{'email'}, $roles) if $userinfo->{'email'};
