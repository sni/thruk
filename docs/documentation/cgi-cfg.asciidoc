...
{% include new_since.ad version="3.28" %}
...
This can be overridden for a single host or service with the
`_THRUK_ESCAPE_HTML_TAGS` custom variable. A value of `0` renders the plugin
output as raw html, a value of `1` keeps it escaped. A service value takes
precedence over a host value.

NOTE: Disabling escaping renders plugin output without sanitizing it. Only use
this for plugins you trust. Plugin HTML is rendered inside a sandboxed iframe,
so plugin CSS cannot affect the rest of the interface and plugin Javascript is
not executed.
...
ex.:
  ...
  # render raw html plugin output for a single service
  define service {
    ...
    _THRUK_ESCAPE_HTML_TAGS 0
  }
...
A comma-delimited list of usernames that have the permission to show
extended information in the debug profiles, ex.: livestatus queries.

ex.:

  authorized_for_debug_profiles = thrukadmin
