...
|`age(<column>)`
|`age(last_check)`
|Subtract value from current unix timestamp to get the age in seconds.

|`calc(<column>, <operator>, <value>)`
|`calc(last_check, '*', 1000)`
...
|Convert amount of seconds into human readable duration.

|`fmt("format", <column>|<text>, ...)`
|`fmt("%.1f", execution_time)`
|Applies format string to column value(s).
...
|`s(<column>, <regex>, <replacement>)`
|`s(host_name, '/\..*$/', '')`
|Apply regex replacement. Note: you cannot use backreferences like `$1` in the replacement string for security reasons.

|`substr(<column>, <offset> [, <length>])`
|`substr(name, 0, 3)`
|Extract substring.

|`unit(<column>, <unit>)`
|`unit(calc(rta, '*', 1000), "s")`
|Set the unit for this column.
...
|`utc(<column>, [<format>])`
|`utc(last_check, "%Y-%m-%d %H:%M:%S %Z")`
|Convert unix timestamp to UTC date.

|`date(<column>, [<format>])`
|`date(last_check, "%Y-%m-%d %H:%M:%S %Z")`
|Convert unix timestamp to date in local timezone.

|`hoststate(<column>)`
|`hoststate(state)`
|Convert numeric host state into text representation, ex.: 0 -> UP, 1 -> DOWN, ...

|`servicestate(<column>)`
|`servicestate(state)`
|Convert numeric service state into text representation, ex.: 0 -> OK, 1 -> WARNING, ...
|===========================
