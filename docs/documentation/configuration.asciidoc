...
-------
  ...
  * cgi.cfg                        # the cgi.cfg is read on every request
...
ex.:
  ...
  command_enabled = 1-4
  command_enabled = 33, 34

results in:

  command_enabled = 1-4, 33, 34

However, if you want to override a list value completely, you can
reset the list by setting it to an empty value:
...
=== api_keys_enabled
{% include new_since.ad version="2.24" %}
Using api keys can be disabled by setting this to 0. You cannot use api
keys at all if this is disabled.

Note: this value cannot be overridden on a per user/group basis because it is
used on pre-authentication stage. If you want users to not create new keys, use
...
This sets how the home link is opened.
By default this is set up open in a new tab (_blank).
...
Timeout after which `thruk -a graph` fails. Defaults to 30.
...
for example to send commands. Use comma separated list or
...
Sets the minimum length a password must have for users changing their passwords.
...
Show the basic auth user / password form. Enabled when using cookie auth.
You may want to set this to 0 if you only use oauth2 authentication.
Set it to 2, if you want to only show the oauth2 authentication but also have
a toggle button, which can display the basic auth login form as a fallback.
...
If a page takes longer to render than this amount of seconds, a
...
Defines an optional separate logfile with some extra audit relevant log
...
therefore relative to the current selected theme. You could set a fixed
path here.
...
It is enabled by default, but can have significant performance impact in larger
setups.
...
=== search_hosts_first
{% include new_since.ad version="3.30" %}

When searching from the navigation bar, try to search hosts first.
...
=== datetime_format
Default time format. Use POSIX format.
...
Default time format for todays date. Can be useful if you want a
...
Determines which columns and the order of the displayed columns.
See an example on the link:dynamic-views.html[Dynamic Views] page.
Use ex.: `peer_name:site` to always show the site name column or optional
only if there is more than one backend with the `?` as in `peer_name?:site`.
 ...
 * 1 = show them for contacts with the role: authorized_for_configuration_information (default)
...
variable. This is a simple text replacement with no regular expressions. This also
supports the optional base64 encoding.
...
Color complete status line with status color or just the status
itself.
...
=== hide_passive_icon
Normally passive checks would be displayed with a passive icon
...
Set sitepanel to list/compact/collapsed/tree/auto/off to change the default behavior.
...
the naemon process. Cache supports only Mysql. Preferred
...
Define wether logcache will be bypassed if the start / end time of a log queries
is outside the range of the cache.

    - `0`: never, only use cached logs and return empty result if outside cached range. (default)
    - `1`: partially, bypass logcache if start and end are outside cache range, otherwise return partially result.
...
There are some useful environment variables set before the script is started:
...
replaces only the the fetching logs part from the builtin logcache.
...
There are some useful environment variables set before the script is started to
...
will disable locks all together.
...
ex.:
  ...
  <auth_oauth>
    <provider name>
      ...
      login_field   = login                                       # Hash key from user info to get the actual username from. If not set, Thruk will try 'login', then 'email'
      groups_field  = groups                                      # Hash key from user info to fetch teams from. If not set, Thruk will try 'groups'
      #enable_pkce  = 0                                           # enable oauth 2.1 pkce workflow if set to 1. Disabled by default
      #https_proxy  = http://<server>:<port>                      # optional proxy to use for accessing URLs from above
      #ssl_verify_hostnames = 1                                   # skip ssl hostname verification when set to 0
      #autologin    = 0                                           # automatically select and submit this provider when set to 1
    </provider>
  </auth_oauth>
...
and therefore allow you to reschedule passive checks directly from the problems
page.
...
=== rest_commands_ignore_partial_errors

When sending commands to multiple backends, they might fail on some backends
but succeed on others. This option allows you to ignore partial errors. If
set, the result will be 200 OK if at least one command has been successfully
delivered.

ex.:

  rest_commands_ignore_partial_errors = 0
...
Sample menu with two items and a separator:
...
A menu has the following attributes:
     ...
     ** `icon`          icon for the menu itself. You can use `{% raw %}{{theme}}{% endraw %}` as placeholder in the url
                        ...
                        An Icon can be either:
                          - a path to an image
                          - an unicon icon when starting with `uil-` (link:https://iconscout.com/unicons/explore/line[list of unicon icons])
                          - a font-awesome icon when starting with `fa-` (link:https://fontawesome.com/search?o=r&m=free&s=solid[list of font awesome icons])
...
A single `"-"` item can be used as a menu item separator.
...
The menu item can have the following attributes:
     ...
     ** `action`        url or action which will be run or opened. This can either be a
...
The following variables can be used in the action attribute when specifying a server action or a http(s) url.
...
by the filename later. For example if you filename is `/etc/thruk/action_menus/some_menu.json` then
you can access the menu by the name `some_menu`.
...
{% raw %}
  *thruk_local.conf:*
    ...
    <action_menu_items>
        standard_menu      = file:///etc/thruk/standard_menu.json
    </action_menu_items>
{% endraw %}
.....
...
{% raw %}
  *thruk_local.conf:*
    ...
    <action_menu_items>
        standard_menu = {"icon":"/thruk/themes/{{theme}}/images/dropdown.png",\
                        "title": "test title",\
                        "menu":[\
                           {"icon":"uil-redo",\
                            "label":"refresh",\
  ...
  *host.cfg:*
    ...
    define host {
      host_name            localhost
      ...
      _THRUK_ACTION_MENU   standard_menu
    }
{% endraw %}
.....
...
a `/etc/thruk/action_menus/host_menu.json` file.
...
{% raw %}
    <action_menu_items>
        host_menu     = file:///etc/thruk/action_menus/host_menu.json
    </action_menu_items>
{% endraw %}
.....
...
=== action_menu_apply
{% include new_since.ad version="1.86" %}
With apply rules adding action menus is made easy and independent from the actual
...
ex.:
  ...
  <action_menu_apply>
    ...
    service_menu = .*;.+$           # matches all services on all hosts
    special_menu = ^Host;Service$   # matches an exact service on one host only
    cpu_menu     = .*;CPU           # matches all services starting with 'CPU'
    host_menu    = ^Host            # matches all hosts starting with 'Host' (incl. services)
    demo_menu    = ^Demo;.*$        # matches all services on the 'Demo' host
    demo_menu    = ^Test;           # a menu name can be used multiple times
  </action_menu_apply>
...
and *not* just a shell command. Therefore you do not have to use quotes because the arguments
...
=== peer
     ...
     ** `authoritative` When having multiple backends, set this to true to gather authorization information only from this backend(s).
     ...
     ** `options`
        *** `peer`            address of this connection. (can be used multiple times when using LMD)
        ...
        *** `fallback_peer`   fallback connection address. (optional, can be used multiple times, used when all peer addresses fail, only works with LMD)
        ...
        *** `host_name`       ssh host name (currently only used from the node-control module)
        *** `site_name`       ssh user name (currently only used from the node-control module)
     ** `configtool`
        ...
        *** `obj_dir`         Path to your objects. Enables the objects editor.
                              Reads all *.cfg from this folder and all subfolder.
     ...
     ** `lmd_options`
        ...
        *** `...`             any attribute will be just passed into the connection section of the lmd.ini, see example below.
...
Read details on the link:/documentation/configuration.html#peer[configtool section].
...
=== default_dashboard
Contains a comma separated list of dashboards you want to show by default.
...
Find more wms provider here: http://wiki.openstreetmap.org/wiki/WMS#OSM_WMS_Servers
  ...
  <Component Thruk::Plugin::Panorama>
    ...
    wms_provider = <providername>   = ["<wms-url>", {"layers": "<layername>"}", {"properties": "..."}]
  </Component>
...
are either exported locally by executing php or remotely fetched with
...
=== config_backend
Use this backend to store the actual objects. Must be a backend of type http.
...
to 1 disables parallelization. Setting it to 0 tries to autodetect a
suitable value.

Automatic bp calculations will be disabled if set to zero.

=== offline_grace_time
Set the time (in seconds) how long a business process will wait for
offline backends to recover. During this time the business process
will keep its last known state.
...
=== default_tab
Choose which tab should be displayed by default. Only useful if several http
backends are in use.

Valid values are:

    * `local` - Local BPs (default)
    * `remote` - Show remote BPs
    * `all` - Show all BPs
...
=== Roles
The roles directive overrides specific config settings for one role only.
Role overrides are applied in alphabetical order. For example, if you have
a user in group 'authorized_for_reports' and in 'authorized_for_admin', he
will get all overrides from both groups, but the 'reports' overrides will
be superseded by the 'admin'.

ex.:

...................................
    # disable creating api keys
    max_api_keys_per_user = 0

    # <Role authorized_for_admin> would also work
    <Role admin>
        # enable api keys for admins again
        max_api_keys_per_user = 10
    </Role>
...................................
