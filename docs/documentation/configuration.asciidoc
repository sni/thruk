...
ex.:
  ...
  <auth_oauth>
    <provider name>
      ...
      groups_field  = groups                                      # Hash key from userinfo to fetch teams from. If not set, Thruk will try 'groups'
      ...
      #ssl_verify_hostnames = 1                                   # skip ssl hostname verfication when set to 0
...
=== peer
     ...
     ** `authoritative` When having multiple backends, set this to true to gather autorization information only from this backend(s).
