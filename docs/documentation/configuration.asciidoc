...
=== rest_skip_command_partial_errors

When sending commands to multiple backends, might fail on some backends but
succeed on others. This option allows you to skip partial errors. If set, the
result will be 200 OK if at least one command has been successfully delivered.

ex.:

  rest_skip_command_partial_errors = 0
